...
public class ChessBoard extends JFrame implements MouseListener, MouseMotionListener {
	...
	/* Pane field so we can get implement mousepressed and mouse dragged, and mouse released
	 * ChessBoard will be a JPanel
	 * JLabel for each chesspiece so we can input ImageIcon
	 * x and y will hold the previous locations for mouseclicked
	 */ 
	JLayeredPane pane;
	JPanel chessBoard;
	JLabel chessPiece;
 ...
 //NOTE ALL IMAGES FOR THE PIECE OR JLABELS
	public void mousePressed(MouseEvent e){
		...
		//store the chessPiece in a chessPiece and sets the location image from the mouse pointer
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(chessPiece == null) 
			...
			if (pieceImage instanceof JLabel){
				Container tile = pieceImage.getParent(); //gets the parent componenet of the piece which is the tile
				tile.remove(0); // removes whichever piece was already there
				tile.add(chessPiece); //adds the piece onto the tile
			}
